  ...
  void * untyped_ros_message)
{
  ...
  // Field name: deletes
  {
    ...
    if (!unique_identifier_msgs__msg__UUID__Sequence__init(&ros_message->deletes, size)) {
      return "failed to create array for field 'deletes'";
    }
    auto array_ptr = ros_message->deletes.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _GeographicMapChanges__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_geographic_msgs__msg__GeographicMapChanges(
    full_bounded, 0);
}
