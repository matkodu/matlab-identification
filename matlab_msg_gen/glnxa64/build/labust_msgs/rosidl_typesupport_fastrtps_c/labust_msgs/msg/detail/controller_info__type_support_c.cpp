// generated from rosidl_typesupport_fastrtps_c/resource/idl__type_support_c.cpp.em
// with input from labust_msgs:msg/ControllerInfo.idl
// generated code does not contain a copyright notice
#include "labust_msgs/msg/detail/controller_info__rosidl_typesupport_fastrtps_c.h"


#include <cassert>
#include <limits>
#include <string>
#include "rosidl_typesupport_fastrtps_c/identifier.h"
#include "rosidl_typesupport_fastrtps_c/wstring_conversion.hpp"
#include "rosidl_typesupport_fastrtps_cpp/message_type_support.h"
#include "labust_msgs/msg/rosidl_typesupport_fastrtps_c__visibility_control.h"
#include "labust_msgs/msg/detail/controller_info__struct.h"
#include "labust_msgs/msg/detail/controller_info__functions.h"
#include "fastcdr/Cdr.h"

#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-parameter"
# ifdef __clang__
#  pragma clang diagnostic ignored "-Wdeprecated-register"
#  pragma clang diagnostic ignored "-Wreturn-type-c-linkage"
# endif
#endif
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif

// includes and forward declarations of message dependencies and their conversion functions

#if defined(__cplusplus)
extern "C"
{
#endif

#include "rosidl_runtime_c/string.h"  // external_topic, manual_topic, state_topic, tracking_topic
#include "rosidl_runtime_c/string_functions.h"  // external_topic, manual_topic, state_topic, tracking_topic

// forward declare type support functions


using _ControllerInfo__ros_msg_type = labust_msgs__msg__ControllerInfo;

static bool _ControllerInfo__cdr_serialize(
  const void * untyped_ros_message,
  eprosima::fastcdr::Cdr & cdr)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  const _ControllerInfo__ros_msg_type * ros_message = static_cast<const _ControllerInfo__ros_msg_type *>(untyped_ros_message);
  // Field name: manual_topic
  {
    const rosidl_runtime_c__String * str = &ros_message->manual_topic;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: external_topic
  {
    const rosidl_runtime_c__String * str = &ros_message->external_topic;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: state_topic
  {
    const rosidl_runtime_c__String * str = &ros_message->state_topic;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: tracking_topic
  {
    const rosidl_runtime_c__String * str = &ros_message->tracking_topic;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
    if (str->data[str->size] != '\0') {
      fprintf(stderr, "string not null-terminated\n");
      return false;
    }
    cdr << str->data;
  }

  // Field name: state
  {
    cdr << ros_message->state;
  }

  return true;
}

static bool _ControllerInfo__cdr_deserialize(
  eprosima::fastcdr::Cdr & cdr,
  void * untyped_ros_message)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  _ControllerInfo__ros_msg_type * ros_message = static_cast<_ControllerInfo__ros_msg_type *>(untyped_ros_message);
  // Field name: manual_topic
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->manual_topic.data) {
      rosidl_runtime_c__String__init(&ros_message->manual_topic);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->manual_topic,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'manual_topic'\n");
      return false;
    }
  }

  // Field name: external_topic
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->external_topic.data) {
      rosidl_runtime_c__String__init(&ros_message->external_topic);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->external_topic,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'external_topic'\n");
      return false;
    }
  }

  // Field name: state_topic
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->state_topic.data) {
      rosidl_runtime_c__String__init(&ros_message->state_topic);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->state_topic,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'state_topic'\n");
      return false;
    }
  }

  // Field name: tracking_topic
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->tracking_topic.data) {
      rosidl_runtime_c__String__init(&ros_message->tracking_topic);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->tracking_topic,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'tracking_topic'\n");
      return false;
    }
  }

  // Field name: state
  {
    cdr >> ros_message->state;
  }

  return true;
}

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_labust_msgs
size_t get_serialized_size_labust_msgs__msg__ControllerInfo(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  const _ControllerInfo__ros_msg_type * ros_message = static_cast<const _ControllerInfo__ros_msg_type *>(untyped_ros_message);
  (void)ros_message;
  size_t initial_alignment = current_alignment;

  const size_t padding = 4;
  const size_t wchar_size = 4;
  (void)padding;
  (void)wchar_size;

  // field.name manual_topic
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->manual_topic.size + 1);
  // field.name external_topic
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->external_topic.size + 1);
  // field.name state_topic
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->state_topic.size + 1);
  // field.name tracking_topic
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->tracking_topic.size + 1);
  // field.name state
  {
    size_t item_size = sizeof(ros_message->state);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }

  return current_alignment - initial_alignment;
}

static uint32_t _ControllerInfo__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_labust_msgs__msg__ControllerInfo(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_labust_msgs
size_t max_serialized_size_labust_msgs__msg__ControllerInfo(
  bool & full_bounded,
  size_t current_alignment)
{
  size_t initial_alignment = current_alignment;

  const size_t padding = 4;
  const size_t wchar_size = 4;
  (void)padding;
  (void)wchar_size;
  (void)full_bounded;

  // member: manual_topic
  {
    size_t array_size = 1;

    full_bounded = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: external_topic
  {
    size_t array_size = 1;

    full_bounded = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: state_topic
  {
    size_t array_size = 1;

    full_bounded = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: tracking_topic
  {
    size_t array_size = 1;

    full_bounded = false;
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += padding +
        eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
        1;
    }
  }
  // member: state
  {
    size_t array_size = 1;

    current_alignment += array_size * sizeof(uint8_t);
  }

  return current_alignment - initial_alignment;
}

static size_t _ControllerInfo__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_labust_msgs__msg__ControllerInfo(
    full_bounded, 0);
}


static message_type_support_callbacks_t __callbacks_ControllerInfo = {
  "labust_msgs::msg",
  "ControllerInfo",
  _ControllerInfo__cdr_serialize,
  _ControllerInfo__cdr_deserialize,
  _ControllerInfo__get_serialized_size,
  _ControllerInfo__max_serialized_size
};

static rosidl_message_type_support_t _ControllerInfo__type_support = {
  rosidl_typesupport_fastrtps_c__identifier,
  &__callbacks_ControllerInfo,
  get_message_typesupport_handle_function,
};

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, labust_msgs, msg, ControllerInfo)() {
  return &_ControllerInfo__type_support;
}

#if defined(__cplusplus)
}
#endif
